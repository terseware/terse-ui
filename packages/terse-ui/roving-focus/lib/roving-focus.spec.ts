...
describe('RovingFocus', () => {
  ...
  describe('initial tab stop', () => {
    ...
    it('skips hard-disabled items when choosing the initial tab stop', async () => {
      await render(
        `<div terseRovingFocus role="toolbar" aria-label="actions">
           <button terseRovingFocusItem disabled>Apple</button>
           <button terseRovingFocusItem>Banana</button>
           <button terseRovingFocusItem>Cherry</button>
         </div>`,
        {imports: IMPORTS},
  ...
  describe('vertical navigation', () => {
    ...
    it('does NOT set aria-orientation when the host has no orientation-bearing role', async () => {
      ...
      const {container} = await render(
        `<div terseRovingFocus>
           <button terseRovingFocusItem>One</button>
           <button terseRovingFocusItem>Two</button>
         </div>`,
        {imports: IMPORTS},
      );
      const group = container.querySelector('[terseRovingFocus]');
  ...
  describe('hard-disabled items', () => {
    it('navigation skips hard-disabled items', async () => {
      await render(
        `<div terseRovingFocus role="toolbar" aria-label="actions">
           <button terseRovingFocusItem>Apple</button>
           <button terseRovingFocusItem disabled>Banana</button>
           <button terseRovingFocusItem>Cherry</button>
         </div>`,
        {imports: IMPORTS},
      );
  ...
  // -------------------------------------------------------------------------
  // Group disabled via terseRovingFocus=false
  // -------------------------------------------------------------------------
  ...
  describe('dynamic items', () => {
    it('promotes a new first item to the tab stop when items change', async () => {
      @Component({
        ...
        imports: IMPORTS,
        template: `
          <div aria-label="actions" role="toolbar" terseRovingFocus>
            @for (label of labels(); track label) {
              <button terseRovingFocusItem>{{ label }}</button>
            }
          </div>
        `,
      })
  ...
  describe('edge cases', () => {
    it('does not throw when navigating an empty group', async () => {
      const {container} = await render(`<div terseRovingFocus aria-label="empty"></div>`, {
        imports: IMPORTS,
      });
      const group = container.querySelector('[terseRovingFocus]') as HTMLElement;
      group.tabIndex = 0;
      group.focus();
    ...
    it('group with only hard-disabled items has no tab stop', async () => {
      await render(
        `<div terseRovingFocus role="toolbar" aria-label="actions">
           <button terseRovingFocusItem disabled>One</button>
           <button terseRovingFocusItem disabled>Two</button>
         </div>`,
        {imports: IMPORTS},
      );
  ...
  describe('axe a11y', () => {
    ...
    it('group scoped by aria-label is queryable by that label (group landmark semantics)', async () => {
      ...
      const {container} = await render(
        `<div terseRovingFocus orientation="horizontal" role="toolbar" aria-label="Editor tools">
           <button terseRovingFocusItem aria-label="Bold">B</button>
           <button terseRovingFocusItem aria-label="Italic">I</button>
           <button terseRovingFocusItem aria-label="Underline">U</button>
         </div>`,
        {imports: IMPORTS},
      );
