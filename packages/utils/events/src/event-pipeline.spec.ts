...
describe('eventPipeline', () => {
  ...
  describe('preventDefault()', () => {
    it('calls event.preventDefault()', async () => {
      ...
      class Host {
        constructor() {
          inject(TestOnKeyDown).intercept(({preventDefault}) => preventDefault());
        }
      }
    ...
    it('idempotent — multiple preventDefault() calls are safe', async () => {
      ...
      class Host {
        constructor() {
          const pipeline = inject(TestOnKeyDown);
          pipeline.intercept(({preventDefault}) => preventDefault());
          pipeline.intercept(({next, preventDefault}) => {
            preventDefault();
            next();
          });
        }
    ...
    it('defaultPrevented() reflects earlier preventDefault() calls in the same dispatch', async () => {
      ...
      class Host {
        constructor() {
          const pipeline = inject(TestOnKeyDown);
          pipeline.intercept(({defaultPrevented}) => {
  ...
  describe('composition: outer directive intercepts before inner primitive', () => {
    ...
    it('outer intercepts Space; inner still handles Enter via next()', async () => {
      ...
      class Btn {
        readonly el = inject(ElementRef).nativeElement as HTMLElement;
        constructor() {
          inject(TestOnKeyDown).intercept(({event, preventDefault}) => {
            innerKeydownSpy(event.key);
            if (event.key === ' ') preventDefault(); // default: prevent scroll
          });
      ...
      class MenuItem {
        constructor() {
          inject(TestOnKeyDown).intercept(({event, next, preventDefault, halt}) => {
            if (event.key === ' ') {
              preventDefault();
              menuItemActivated();
              halt();
          ...
          inject(TestOnKeyUp).intercept(({event, next, preventDefault}) => {
            if (event.key === ' ') {
              preventDefault();
  ...
  describe('composition: soft-disabled blocks activation but allows focus keys', () => {
    it('blocks Enter/Space; allows Tab through', async () => {
      ...
      class Btn {
        ...
        constructor() {
          inject(TestOnKeyDown).intercept(({event, halt, preventDefault}) => {
