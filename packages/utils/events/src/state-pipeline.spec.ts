...
describe('statePipeline', () => {
  ...
  describe('execution order', () => {
    ...
    it('defaultHandler runs innermost (last in execution)', () => {
      TestBed.runInInjectionContext(() => {
        const order: string[] = [];
        const p = statePipeline(0, {
          defaultHandler: ({next}) => {
        ...
        p();
        expect(order).toEqual(['intercepted', 'default']);
      });
    });
  });

  describe('next()', () => {
    it('forwards incoming when called with no args', () => {
      TestBed.runInInjectionContext(() => {
        const p = statePipeline(7);
        let seen: number | undefined;
        p.intercept(({incoming, next}) => {
          seen = incoming;
          return next();
        });
        expect(p()).toBe(7);
    ...
    it('transforms the value passed inward', () => {
      TestBed.runInInjectionContext(() => {
        const p = statePipeline(1);
        p.intercept(({incoming, next}) => next(incoming + 1)); // inner
        p.intercept(({incoming, next}) => next(incoming * 10)); // outer
        // outer runs first with incoming=1, calls next(10); inner runs with incoming=10, calls next(11).
        expect(p()).toBe(11);
      });
    });
  ...
  describe('finalize', () => {
    ...
    it('receives the value produced by the chain', () => {
      TestBed.runInInjectionContext(() => {
        const p = statePipeline(1, {finalize: (v) => v + 1});
        p.intercept(({incoming, next}) => next(incoming * 10));
        expect(p()).toBe(11);
      });
    });
  });
  ...
  describe('override', () => {
    it('injects the value into the inner chain', () => {
      TestBed.runInInjectionContext(() => {
        const p = statePipeline<number>(1);
        let seen: number | undefined;
        p.intercept(({incoming, next}) => {
          seen = incoming;
          return next();
  ...
  describe('intercept cleanup', () => {
    ...
    it('auto-removes when the surrounding injection context is destroyed', () => {
      const parent = TestBed.inject(EnvironmentInjector);
      const p = runInInjectionContext(parent, () => statePipeline(0));
      const child = createEnvironmentInjector([], parent);
      runInInjectionContext(child, () => p.intercept(() => 42));
      expect(p()).toBe(42);
      child.destroy();
      expect(p()).toBe(0);
    });
  ...
  describe('statePipeline.deep', () => {
    ...
    it('supports intercepted transforms', () => {
      TestBed.runInInjectionContext(() => {
        const p = statePipeline.deep({count: 1});
        p.intercept(({next}) => next({count: 99}));
